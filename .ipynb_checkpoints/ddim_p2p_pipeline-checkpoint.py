...
def register_attention_control(model, controller):
    """
    将自定义的 Attention Controller 注册到 Diffusion 模型中。
    原理：遍历 UNet 的子模块，找到 Attention 层，替换其 forward 方法。
    """
    def ca_forward(self, place_in_unet):
        ...
        def forward(x, context=None, mask=None, **kwargs):
            ...
            # --- 关键点 ---
            # 调用外部传入的 controller 对 attention map 进行处理（保存或替换）
...
def p2p_guidance_diffusion_step(model, controller, latents, context, t, guidance_scale, low_resource=False):
    """单步 Diffusion 生成，并包含 Controller 回调"""
    if low_resource:
    ...
    # 调用 Controller 的 step_callback (主要用于 Local Blend)
